...
public class StudyCaseFromDBWithExplain extends Frame  {
    /**
	 * 
	 */
	...
	private static final int ADDCOUNT=4;//手动输入增加节点的次数
	private static final int LINEFIND=24;//找介数最大的边的条数
		...
		public static int[] findGroup(){
           ...
           // print(graph1,p);//输出当前得到的图的矩阵
			...
			vv.getRenderContext().setEdgeFillPaintTransformer(
					new Transformer<RelationLink, Paint>() {
						public Paint transform(RelationLink p) {
							for(int nn=0;nn<red.length;nn++)
							if (p.equals(relationLinkArray[red[nn]]))
								return Color.RED;
							    return Color.GREEN;
							   // return null;
